...
> 모던 C++: `auto`로 타입 추론하고 `{}` 균일 초기화로 좁힘 변환을 차단하는 습관은 입문 단계부터 권장된다 → [[모던 C++ 보강 — C++11 이후]]
...
## 연습문제
1. 다음 식별자 중 유효한 것을 고르고 무효한 것은 이유를 적어라: `2score`, `_total`, `class`, `userName`, `my-var`.
2. `int a = 7, b = 2;`일 때 `a / b`, `a % b`, `static_cast<double>(a) / b`의 값을 각각 손으로 계산하라.
3. 이름과 나이를 입력받아 "내년이면 N세입니다"를 출력하는 프로그램을 작성하라. 변수는 `{}` 초기화를 사용할 것.
> 힌트: 1번에서 키워드와 시작 문자 규칙, 허용 문자(`-` 불가)를 따져 보라.

## 관련 개념
- [[OOP Ch01 컴퓨터와 프로그래밍 언어]] — 컴파일 파이프라인
- [[OOP Ch03 표현식과 문장]] — 변수·리터럴로 식을 만드는 단계
- [[SA Ch02 C++ 언어 입문]] — 구조적 접근 트랙의 같은 주제
