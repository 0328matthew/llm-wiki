...
## 연습문제
1. 팩토리얼과 피보나치를 재귀·반복 두 방식으로 구현하고 호출 횟수를 비교하라.
2. 하노이탑에서 원반 n개를 옮기는 이동 순서와 총 이동 횟수를 출력하라.
3. 재귀만으로 문자열을 뒤집는 함수를 작성하라.
> 힌트: 재귀는 반드시 기저 사례(종료 조건)를 먼저 정의해야 무한 재귀를 피한다.

## 관련 개념
- [[OOP Ch18 자료구조 입문]] — 스택/트리, 재귀와 자연스럽게 맞물림
- [[Tool Use]] — 큰 입력의 정확한 수치는 실행으로 위임
- [[비선형 방정식 풀이]] — 이분법 등 분할 사고와 통하는 면
