...
## 연습문제
1. 싱글턴 `Logger`를 구현하고, 서로 다른 곳에서 얻은 인스턴스의 주소가 같은지 확인하라.
2. 전략(Strategy) 패턴을 `std::function`/람다로 구현해 정렬 기준을 런타임에 교체하라.
3. 옵서버 패턴으로 한 주체의 값 변경이 여러 구독자에게 통지되는 예제를 작성하라.
> 힌트: 2번은 가상함수 계층 없이 람다만으로도 전략 교체가 된다.

## 관련 개념
- [[OOP Ch19 표준 템플릿 라이브러리 — STL]] — 반복자는 Iterator 패턴의 실체
- [[OOP Ch01 컴퓨터와 프로그래밍 언어]] — OOP 패러다임의 출발점
- [[모던 C++ 보강 — C++11 이후]] — 람다·`std::function`으로 패턴을 더 가볍게
