...
### 템플릿 인스턴스화와 컴파일 모델
- 템플릿은 그 자체로 코드가 아니라 **코드를 찍어내는 틀**이다 — 컴파일러는 `Stack<int>`처럼 **실제 사용된 타입**을 보고 비로소 해당 버전을 생성(인스턴스화)한다.
- 인스턴스화 시점에 **템플릿의 정의 전체**가 보여야 하므로, 선언만 헤더에 두고 정의를 별도 `.cpp`에 두면 다른 번역 단위에서 인스턴스를 만들 수 없어 `undefined reference` 링크 오류가 난다.
- 그래서 함수·클래스 템플릿은 **선언과 정의를 모두 헤더에** 둔다(또는 명시적 인스턴스화로 우회). STL 헤더가 무거운 이유이기도 하다.

> 모던 C++: C++20의 **concepts/제약(requires)** 로 템플릿 매개변수에 조건을 걸어, 인스턴스화 실패 시의 난해한 오류 대신 명확한 진단을 얻을 수 있다 → [[모던 C++ 보강 — C++11 이후]]
...
## 연습문제
1. 임의 타입의 두 값을 받아 작은 쪽을 돌려주는 함수 템플릿 `myMin`을 작성하고 `int`·`double`·`std::string`으로 호출해 보라.
   > 힌트: `<` 연산만 요구되므로 비교 가능한 타입이면 모두 동작한다.
2. 고정 크기 `N`을 비타입 매개변수로 받는 클래스 템플릿 `RingBuffer<T, N>`를 만들어 `push`/`front` 동작을 구현하라.
3. `Printer<T>` 일반 템플릿을 만들고 `bool`에 대해 `"true"/"false"`를 출력하도록 **명시적 특수화**하라.
   > 힌트: `template <> class Printer<bool> { ... };` 형태.

## 관련 개념
- [[OOP Ch14 예외 처리]] — STL 컨테이너의 예외(`at()`의 `out_of_range`)
- [[OOP Ch16 입출력 스트림]] — 스트림 역시 `basic_istream<charT>` 템플릿 기반
- [[모던 C++ 보강 — C++11 이후]] — concepts/제약으로 템플릿 매개변수 조건 명시(C++20)
