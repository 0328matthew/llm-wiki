...
⬅︎ [[SA Ch08 배열]] · [[CS-MOC]] · 다음 → [[SA Ch10 문자열]]
...
> 모던 C++: `new`/`delete`로 직접 메모리를 관리하는 대신 `std::unique_ptr`/`std::shared_ptr`(스마트 포인터)로 소유권을 명시하면 누수·이중 해제·dangling을 컴파일러와 소멸자가 막아 준다. → [[모던 C++ 보강 — C++11 이후]]
...
## 연습문제
1. 포인터를 이용해 두 변수의 값을 교환하는 함수를 작성하라.
2. `new[]`로 동적 배열을 할당해 입력을 받고 합을 구한 뒤 `delete[]`로 해제하라.
3. 인덱스 `[]` 없이 포인터 산술(`*(p+i)` 또는 `p++`)만으로 배열을 순회·출력하라.
> 힌트: 2번에서 `new[]`는 반드시 `delete[]`와 짝을 맞춰야 한다.

## 관련 개념
- [[SA Ch08 배열]] — 배열 이름이 곧 주소
- [[SA Ch11 유도 자료형 — 열거형·구조체·공용체]] — 구조체 포인터와 `->`
- [[SA Ch13 클래스의 고급 기능]] — 깊은 복사가 필요한 이유
- [[OOP Ch09 참조·포인터·메모리 관리]] — 객체지향 트랙의 같은 주제(참조·Rule of Three까지)
