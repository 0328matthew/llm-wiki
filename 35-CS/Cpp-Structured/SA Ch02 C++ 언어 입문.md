...
## 연습문제
1. 섭씨 온도를 `double`로 입력받아 화씨(`F = C * 9 / 5 + 32`)로 변환해 출력하는 프로그램을 작성하라. 정수 나눗셈 함정에 주의할 것.
2. 사용자에게 반지름을 입력받아 원의 둘레와 넓이를 출력하라. `const double PI`를 사용하라.
3. 두 정수를 입력받아 몫과 나머지를 정수로, 실수 나눗셈 결과를 소수로 함께 출력하라.
> 힌트: 2번에서 `PI`를 상수로 두면 의미가 분명해지고 오타로 인한 값 변경을 막는다.

## 관련 개념
- [[SA Ch03 C++ 프로그램의 구조]] — 표현식·연산자로 데이터 가공
- [[SA Ch07 텍스트 입출력]] — 스트림·포매팅 심화
- [[OOP Ch02 C++ 프로그래밍의 기초]] — 객체지향 트랙의 같은 주제
