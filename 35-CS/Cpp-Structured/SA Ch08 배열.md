...
## 연습문제
1. 정수 배열을 한 번만 순회하며 최댓값·최솟값·평균을 동시에 구하라.
2. 선택 정렬 또는 버블 정렬을 직접 구현하라.
3. 정렬된 배열에서 이분 탐색으로 특정 값의 인덱스를 찾아라(없으면 -1).
> 힌트: 2번은 인접 원소 비교 후 교환을 배열 끝까지 반복한다.

## 관련 개념
- [[SA Ch09 포인터]] — 배열 이름이 곧 주소라는 사실의 정체
- [[SA Ch11 유도 자료형 — 열거형·구조체·공용체]] — 서로 다른 자료형을 묶는 struct
- [[OOP Ch08 배열]] — 객체지향 트랙의 같은 주제
- [[모던 C++ 보강 — C++11 이후]] — 경계 안전한 `std::vector`/`std::array` 권장
